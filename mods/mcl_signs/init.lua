...
local generate_texture = function(lines, signnodename)
	...
	local ypos
	if signnodename == "mcla_signs:wall_sign" then
		ypos = 30
	else
		ypos = 0
	end
...
local function get_rotation_level(facedir, nodename)
	local rl = facedir * 4
	if nodename == "mcla_signs:standing_sign22_5" then
		rl = rl + 1
	elseif nodename == "mcla_signs:standing_sign45" then
		rl = rl + 2
	elseif nodename == "mcla_signs:standing_sign67_5" then
		rl = rl + 3
	end
	return rl
end
...
local sign_groups = {
  cracky = 3, -- universal break group
  oddly_breakable_by_hand = 3, -- makes it breakable by hand
  flammable = 1,
  deco_block = 1,
  attached_node = 1,
}
...
local destruct_sign = function(pos)
	...
	for p=1, #players do
		if vector.distance(players[p]:get_pos(), pos) <= 30 then
			minetest.close_formspec(players[p]:get_player_name(), "mcla_signs:set_text_"..pos.x.."_"..pos.y.."_"..pos.z)
		end
	end
end
...
local update_sign = function(pos, fields, sender, force_remove)
	...
	local nn = n.name
	if nn == "mcla_signs:standing_sign" or nn == "mcla_signs:standing_sign22_5" or nn == "mcla_signs:standing_sign45" or nn == "mcla_signs:standing_sign67_5" then
		sign_info = signtext_info_standing[get_rotation_level(n.param2, nn) + 1]
	elseif nn == "mcla_signs:wall_sign" then
...
local show_formspec = function(player, pos)
	minetest.show_formspec(
		...
		"mcla_signs:set_text_"..pos.x.."_"..pos.y.."_"..pos.z,
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname:find("mcla_signs:set_text_") == 1 then
		local x, y, z = formname:match("mcla_signs:set_text_(.-)_(.-)_(.*)")
...
minetest.register_node(":mcla_signs:wall_sign", {
	description = S("Sign"),
	inventory_image = "mcla_signs_icon.png",
	walkable = false,
	is_ground_content = false,
	wield_image = "mcla_signs_icon.png",
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		elseif wdir == 1 then
			...
			if rotation_level % 4 == 0 then
				nodeitem:set_name("mcla_signs:standing_sign")
			elseif rotation_level % 4 == 1 then
				nodeitem:set_name("mcla_signs:standing_sign22_5")
			elseif rotation_level % 4 == 2 then
				nodeitem:set_name("mcla_signs:standing_sign45")
			elseif rotation_level % 4 == 3 then
				nodeitem:set_name("mcla_signs:standing_sign67_5")
			end
...
local ssign = {
	...
	on_rotate = function(pos, node, user, mode)
		if mode == screwdriver.ROTATE_FACE then
			node.name = "mcla_signs:standing_sign22_5"
			minetest.swap_node(pos, node)
...
minetest.register_node(":mcla_signs:standing_sign", ssign)
...
ssign22_5.on_rotate = function(pos, node, user, mode)
	if mode == screwdriver.ROTATE_FACE then
		node.name = "mcla_signs:standing_sign45"
		minetest.swap_node(pos, node)
	elseif mode == screwdriver.ROTATE_AXIS then
		return false
	end
...
minetest.register_node(":mcla_signs:standing_sign22_5", ssign22_5)
...
ssign45.on_rotate = function(pos, node, user, mode)
	if mode == screwdriver.ROTATE_FACE then
		node.name = "mcla_signs:standing_sign67_5"
		minetest.swap_node(pos, node)
	elseif mode == screwdriver.ROTATE_AXIS then
		return false
	end
...
minetest.register_node(":mcla_signs:standing_sign45", ssign45)
...
ssign67_5.on_rotate = function(pos, node, user, mode)
	if mode == screwdriver.ROTATE_FACE then
		node.name = "mcla_signs:standing_sign"
...
minetest.register_node(":mcla_signs:standing_sign67_5", ssign67_5)
...
minetest.register_craft({
	type = "fuel",
	recipe = "mcla_signs:wall_sign",
	burntime = 10,
})

minetest.register_craft({
	output = 'mcla_signs:wall_sign 3',
	recipe = {
...
minetest.register_lbm({
	...
	nodenames = { "mcla_signs:wall_sign", "mcla_signs:standing_sign", "mcla_signs:standing_sign22_5", "mcla_signs:standing_sign45", "mcla_signs:standing_sign67_5" },
