...
local horse_extra_texture = function(horse)
	...
	if armor then
		if core.get_item_group(armor, "horse_armor") > 0 then
			base = base .. "^" .. core.registered_items[armor]._horse_overlay_image
		end
	end
	if saddle then
...
local horse = {
	...
	do_custom = function(self, dtime)
		...
		if not self.v2 then
			...
			self.accel = 6
			self.terrain_type = 3
			self.driver_attach_at = {x = 0, y = 4.5, z = -1.75}
			self.driver_eye_offset = {x = 0, y = 10 + 3, z = 0}
			self.driver_scale = {x = 0.33, y = 0.33}
		end
	...
	on_die = function(self, pos)
		...
		if self._saddle then
			core.add_item(pos, mobs_mc.items.saddle)
		end
	...
	on_rightclick = function(self, clicker)
		...
		if self.tamed and not self.child and self.owner == clicker:get_player_name() then
			...
			and core.get_item_group(clicker:get_wielded_item():get_name(), "horse_armor") > 0 then


				-- Put on armor and take armor from player's inventory
				local w = clicker:get_wielded_item()
				local armor = core.get_item_group(w:get_name(), "horse_armor")
...
--===========================
--Spawn Function
mobs:spawn_specific("mobs_mc:horse", mobs_mc.spawn.grassland_savanna, {"air"}, 0, core.LIGHT_MAX+1, 30, 15000, 12, mobs_mc.spawn_height.water+3, mobs_mc.spawn_height.overworld_max)
mobs:spawn_specific("mobs_mc:donkey", mobs_mc.spawn.grassland_savanna, {"air"}, 0, core.LIGHT_MAX+1, 30, 15000, 12, mobs_mc.spawn_height.water+3, mobs_mc.spawn_height.overworld_max)
...
if core.settings:get_bool("log_mods") then
	core.log("action", "MC Horse loaded")
end
