...
mobs:register_mob("mobs_mc:villager", {
	...
	on_rightclick = function(self, clicker)
		local inv
		inv = core.get_inventory({type="detached", name="trading_inv"})
		if not inv then
			inv = core.create_detached_inventory("trading_inv", {
				allow_take = function(inv, listname, index, stack, player)
					if listname == "output" then
						inv:remove_item("input", inv:get_stack("wanted", 1))
						core.sound_play("Villageraccept", {to_player = player:get_player_name()})
					end
				...
				on_put = function(inv, listname, index, stack, player)
					if inv:contains_item("input", inv:get_stack("wanted", 1)) then
						...
						core.sound_play("Villagerdeny", {to_player = player:get_player_name()})
					end
				end,
		...
		core.sound_play("Villagertrade", {to_player = clicker:get_player_name()})
		core.show_formspec(clicker:get_player_name(), "tradespec", formspec)
	end,
	
	]]
})

mobs:spawn_specific("mobs_mc:villager", mobs_mc.spawn.village, {"air"}, 0, core.LIGHT_MAX+1, 30, 8000, 4, mobs_mc.spawn_height.water+1, mobs_mc.spawn_height.overworld_max)
