	...
	on_rightclick = function(self, clicker)
		...
		if is_food(item:get_name()) then
			...
			if pr:next(1, 3) == 1 then
				...
				local cat = core.add_entity(self.object:getpos(), "mobs_mc:cat")
				cat:set_yaw(yaw)
	...
	on_spawn = function(self, pos)
		...
		if pr:next(1,7) == 1 then
			...
			for i=1, 2 do
				...
				make_child(core.add_entity(child_pos, "mobs_mc:ocelot"))
			end
		end
	end,
})
...
if core.settings:get_bool("log_mods") then
	core.log("action", "MC Ocelot loaded")
end
