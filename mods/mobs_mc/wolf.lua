...
local is_food = function(itemstring)
	for f=1, #mobs_mc.follow.dog do
		if itemstring == mobs_mc.follow.dog[f] then
			return true
		elseif string.sub(itemstring, 1, 6) == "group:" and core.get_item_group(itemstring, string.sub(itemstring, 7, -1)) ~= 0 then
			return true
		end
	end
	return false
	...
	on_rightclick = function(self, clicker)
		...
		if tool:get_name() == mobs_mc.items.bone then
			...
			if pr:next(1, 3) == 1 then
				...
				dog = core.add_entity(self.object:get_pos(), "mobs_mc:dog")
				dog:set_yaw(yaw)
...
dog.on_rightclick = function(self, clicker)
	...
	elseif is_food(item:get_name()) then
		...
		local eatable = core.get_item_group(item, "eatable")
		if eatable > 0 then
			hp_add = eatable
		...
		self.health = new_hp
		return
	elseif core.get_item_group(item:get_name(), "dye") == 1 then
		-- Dye (if possible)
		for group, _ in pairs(colors) do
			-- Check if color is supported
			if core.get_item_group(item:get_name(), "color_" .. group) == 1 then
				-- Dye collar
...
-- Spawn
mobs:spawn_specific("mobs_mc:wolf", mobs_mc.spawn.wolf, {"air"}, 0, core.LIGHT_MAX+1, 30, 9000, 20, mobs_mc.spawn_height.water+3, mobs_mc.spawn_height.overworld_max)
...
if core.settings:get_bool("log_mods") then
	core.log("action", "MC Wolf loaded")
end
