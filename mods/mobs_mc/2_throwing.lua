...
core.register_node("mobs_mc:arrow_box", {
	drawtype = "nodebox",
	node_box = {
		type = "fixed",
		fixed = {
...
THROWING_ARROW_ENTITY.on_step = function(self, dtime)
		...
		for k, obj in pairs(objs) do
			if obj:get_luaentity() ~= nil then
				if obj:get_luaentity().name ~= "mobs_mc:arrow_entity" and obj:get_luaentity().name ~= "__builtin:item" then
					local damage = 3
					core.sound_play("damage", {pos = pos})
	...
	if self.lastpos.x~=nil then
		if node.name ~= "air" then
			core.sound_play("bowhit1", {pos = pos})
			core.add_item(self.lastpos, 'mobs_mc:arrow')
			self.object:remove()
		end
	end
...
core.register_entity("mobs_mc:arrow_entity", THROWING_ARROW_ENTITY)
...
local throwing_shoot_arrow = function(itemstack, player)
	for _,arrow in ipairs(arrows) do
		if player:get_inventory():get_stack("main", player:get_wield_index()+1):get_name() == arrow[1] then
			...
			local obj = core.add_entity({x=playerpos.x,y=playerpos.y+1.5,z=playerpos.z}, arrow[2])  --mc
			local dir = player:get_look_dir()
			obj:setvelocity({x=dir.x*22, y=dir.y*22, z=dir.z*22})
			obj:setacceleration({x=dir.x*-3, y=-10, z=dir.z*-3})
			obj:setyaw(player:get_look_yaw()+math.pi)
			core.sound_play("throwing_sound", {pos=playerpos})
...
if c("arrow") then
	core.register_craftitem("mobs_mc:arrow", {
		description = S("Arrow"),
...
if c("bow") then
	core.register_tool("mobs_mc:bow_wood", {
		description = S("Bow"),
...
if c("egg") then
	...
	mobs:register_arrow("mobs_mc:egg_entity", {
		...
		hit_node = function(self, pos, node)
			...
			local mob = core.add_entity(pos, "mobs_mc:chicken")
	...
	core.register_craftitem("mobs_mc:egg", {
		description = S("Egg"),
...
if c("snowball") then
	...
	-- Snowball
	core.register_craftitem("mobs_mc:snowball", {
...
if core.settings:get_bool("log_mods") then
	core.log("action", "MC mobs loaded")
end
