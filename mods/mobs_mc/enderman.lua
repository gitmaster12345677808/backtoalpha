...
mobs:register_mob("mobs_mc:enderman", {
	...
	do_custom = function(self, dtime)
		...
		if (self._taken_node == nil or self._taken_node == "") and self._take_place_timer >= self._next_take_place_time then
			...
			local takable_nodes = core.find_nodes_in_area({x=pos.x-2, y=pos.y-1, z=pos.z-2}, {x=pos.x+2, y=pos.y+1, z=pos.z+2}, mobs_mc.enderman_takable)
			if #takable_nodes >= 1 then
				local r = pr:next(1, #takable_nodes)
				local take_pos = takable_nodes[r]
				local node = core.get_node(take_pos)
				local dug = core.dig_node(take_pos)
				if dug then
					...
					if def.sounds and def.sounds.dug then
						core.sound_play(def.sounds.dug, {pos = take_pos, max_hear_distance = 16})
					end
				end
			end
		elseif self._taken_node ~= nil and self._taken_node ~= "" and self._take_place_timer >= self._next_take_place_time then
			...
			local place_pos = vector.subtract(pos, core.facedir_to_dir(core.dir_to_facedir(core.yaw_to_dir(yaw))))
			if core.get_node(place_pos).name == "air" then
				...
				if success then
					...
					if def.sounds and def.sounds.place then
						core.sound_play(def.sounds.place, {pos = place_pos, max_hear_distance = 16})
					end
					self._taken_node = ""
				end
			end
	...
	_do_teleport = function(self)
		...
		local nodes = core.find_nodes_in_area_under_air(vector.subtract(pos, 32), vector.add(pos, 32), {"group:solid", "group:cracky", "group:crumbly"})
		local telepos
		if #nodes > 0 then
	...
	on_die = function(self, pos)
		...
		if self._taken_node ~= nil and self._taken_node ~= "" then
			core.add_item(pos, self._taken_node)
		end
	end,
	water_damage = 8,
	lava_damage = 4,
...
-- End spawn
mobs:spawn_specific("mobs_mc:enderman", mobs_mc.spawn.solid, {"air"}, 0, core.LIGHT_MAX+1, 30, 3000, 12, mobs_mc.spawn_height.end_min, mobs_mc.spawn_height.end_max)
-- Overworld spawn
...
if core.settings:get_bool("log_mods") then
	...
	core.log("action", "MC Enderman loaded")
end
