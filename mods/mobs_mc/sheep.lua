...
if core.get_modpath("mcl_wool") ~= nil then
...
mobs:register_mob("mobs_mc:sheep", {
	...
	on_rightclick = function(self, clicker)
		...
		if item:get_name() == mobs_mc.items.shears and not self.gotten then
			...
			core.add_item(pos, ItemStack(colors[self.color][1].." "..math.random(1,3)))
			self.object:set_properties({
		...
		-- Dye sheep
		if core.get_item_group(item:get_name(), "dye") == 1 and not self.gotten then
			core.log("verbose", "[mobs_mc] " ..item:get_name() .. " " .. core.get_item_group(item:get_name(), "dye"))
			for group, colordata in pairs(colors) do
				if core.get_item_group(item:get_name(), group) == 1
				or core.get_item_group(item:get_name(), "color_"..colordata[3]) == 1 then
...
mobs:spawn_specific("mobs_mc:sheep", mobs_mc.spawn.grassland, {"air"}, 0, core.LIGHT_MAX+1, 30, 15000, 3, mobs_mc.spawn_height.overworld_min, mobs_mc.spawn_height.overworld_max)
...
if core.settings:get_bool("log_mods") then
	core.log("action", "MC Sheep loaded")
end
