...
mobs:register_mob("mobs_mc:creeper", {
		...
		if item:get_name() == mobs_mc.items.flint_and_steel then
			if not core.settings:get_bool("creative_mode") then
				...
				if item:get_count() == 0 and wdef.sound and wdef.sound.breaks then
					core.sound_play(wdef.sound.breaks, {pos = clicker:getpos(), gain = 0.5})
				end
				clicker:set_wielded_item(item)
			end
			self._forced_explosion_countdown_timer = self.explosion_timer
			core.sound_play(self.sounds.attack, {pos = self.object:getpos(), gain = 1, max_hear_distance = 16})
		end
	end,
	...
	on_die = function(self, pos)
		...
		if math.random(1, 200) == 1 then
			...
			core.add_item({x=pos.x, y=pos.y+1, z=pos.z}, mobs_mc.items.music_discs[r])
		end
	end,
	maxdrops = 2,
	drops = {
...
if core.settings:get_bool("log_mods") then
	core.log("action", "MC Creeper loaded")
end
