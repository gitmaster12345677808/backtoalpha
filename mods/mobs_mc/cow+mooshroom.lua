	...
	on_rightclick = function(self, clicker)
		...
		if item:get_name() == mobs_mc.items.bucket and clicker:get_inventory() then
			...
			if inv:room_for_item("main", {name=mobs_mc.items.milk}) then
				...
				pos.y = pos.y + 0.5
				core.add_item(pos, {name = mobs_mc.items.milk})
			end
			return
		end
...
mooshroom_def.on_rightclick = function(self, clicker)
	...
	if item:get_name() == mobs_mc.items.shears then
		local pos = self.object:getpos()
		core.sound_play("shears", {pos = pos})
		core.add_item({x=pos.x, y=pos.y+1.4, z=pos.z}, mobs_mc.items.mushroom_red .. " 5")

		local oldyaw = self.object:getyaw()
		self.object:remove()
		local cow = core.add_entity(pos, "mobs_mc:cow")
		cow:setyaw(oldyaw)
	...
	elseif item:get_name() == mobs_mc.items.bowl and clicker:get_inventory() then
		...
		if inv:room_for_item("main", {name=mobs_mc.items.mushroom_stew}) then
			...
			local pos = self.object:get_pos()
			pos.y = pos.y + 0.5
			core.add_item(pos, {name = mobs_mc.items.mushroom_stew})
		end
	end
	mobs:capture_mob(self, clicker, 0, 5, 60, false, nil)
end
...
-- Spawning
mobs:spawn_specific("mobs_mc:cow", mobs_mc.spawn.grassland, {"air"}, 9, core.LIGHT_MAX+1, 30, 17000, 20, mobs_mc.spawn_height.overworld_min, mobs_mc.spawn_height.overworld_max)
mobs:spawn_specific("mobs_mc:mooshroom", mobs_mc.spawn.mushroom_island, {"air"}, 9, core.LIGHT_MAX+1, 30, 17000, 10, mobs_mc.spawn_height.overworld_min, mobs_mc.spawn_height.overworld_max)
