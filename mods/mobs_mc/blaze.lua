...
mobs:spawn_specific("mobs_mc:blaze", mobs_mc.spawn.nether_fortress, {"air"}, 0, core.LIGHT_MAX+1, 30, 5000, 3, mobs_mc.spawn_height.nether_min, mobs_mc.spawn_height.nether_max)
...
mobs:register_arrow("mobs_mc:blaze_fireball", {
	...
	hit_node = function(self, pos, node)
		if node.name == "air" then
			core.set_node(pos_above, {name=mobs_mc.items.fire})
		else
			...
			if crashnode.name == "air" or
					(core.registered_nodes[crashnode.name].buildable_to and core.get_item_group(crashnode.name, "flammable") >= 1) then
				core.set_node(crashpos, {name=mobs_mc.items.fire})
			end
		end
	end
})
...
if core.settings:get_bool("log_mods") then
	core.log("action", "MC Blaze loaded")
end
